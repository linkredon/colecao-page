...
import { Plus, Bookmark, ExternalLink, Heart, Share2 } from "lucide-react"
...
export default function CardModal() {
  ...
  const getLigaMagicURL = (card: MTGCard): string => {
    // LigaMagic normalmente usa nome da carta na URL, mas precisamos normalizar para seu formato
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl rounded-lg text-card-foreground bg-gray-800/80 backdrop-blur-xl shadow-2xl overflow-hidden border-0">
        <DialogHeader>
          <DialogTitle className="text-xl text-white flex justify-between items-center">
            <span>{card.name}</span>
            <div className="flex gap-2">
              {/* Botões de visualização */}
              <CardViewOptions size="sm" className="mr-2" />
              
              <Button
                size="sm"
                variant="ghost"
                onClick={() => setVisuFavorita(!visuFavorita)}
                className={`h-7 px-2 ${visuFavorita ? 'text-red-400 hover:text-red-300' : 'text-gray-400 hover:text-gray-300'}`}
              >
                <Heart className={`h-4 w-4 ${visuFavorita ? 'fill-red-400' : ''}`} />
              </Button>
              <Button
                size="sm"
                variant="ghost"
                onClick={() => window.navigator.clipboard.writeText(`${window.location.origin}/card/${card.id}`)}
